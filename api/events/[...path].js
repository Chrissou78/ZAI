  ...
  try {
    ...
    if (req.method === 'POST' && segments.length === 2 && segments[1] === 'payment-intent') {
      ...
      if (process.env.STRIPE_CONNECTED_ACCOUNT_ID) {
        ...
        // Off unless explicitly enabled — see the fuller note on the store's
        // PaymentIntent. It switches payment-method config to the connected
        // account's, which broke checkout while that account had none for CHF.
